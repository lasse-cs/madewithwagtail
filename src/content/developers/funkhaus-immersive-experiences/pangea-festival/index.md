---
title: Pangea Festival
first_published_at: '2024-03-27T16:16:06.271813+00:00'
latest_revision_created_at: '2024-03-27T16:16:06.271813+00:00'
site_url: https://www.aboutyoupangea-festival.de/
sector:
  - arts
site_type:
  - events
---

This website is build on top of wagtail to tackle lot's of line up content. We also feed the content from wagtail into an app thanks to wagtails integrated API.
